// Default ONE pass scene shaders
var BloomRendererFragmentShaderSrc =
	[
...
var BloomRendererFragmentShaderSrcY =	
	...
	"       if (at. r + at.b + at.g > 1.5) result += at * weight[int(i)]; ", 
	"       if (bt. r + bt.b + bt.g > 1.5) result += bt * weight[int(i)]; ",  
	//"       if ((abs(at.r - at.b) + abs(at.g - at.r)) < 0.1) result += at * weight[int(i)]; ", 
    //"       if ((abs(bt.r - bt.b) + abs(bt.g - bt.r)) < 0.1) result += bt * weight[int(i)]; ",  
    "   }", 
	"	gl_FragColor = vec4(clamp(result, 0.0, 1.0), 1.0); ", 
	"}"]
		.join('\n');
		
function BloomRenderer (sceneShaderSrc, bloomShaderSrc, bloomShaderSrcX, bloomShaderSrcY) {
	if (sceneShaderSrc == null)
		this.fragmentShaderSrc = DefaultRendererFragmentShaderSrc;
	else
		this.fragmentShaderSrc = sceneShaderSrc;
...
BloomRenderer.prototype = Object.create(new Renderer());

//#TODO: refactor and inherit all these functions
BloomRenderer.prototype.initialize = function (gl) {
	...
	this.initializeShadersBloom(gl);
}

BloomRenderer.prototype.initializeShadersBloom = function (gl) {
	var vertexShader = gl.createShader(gl.VERTEX_SHADER);
	gl.shaderSource(vertexShader, RendererVertexShaderSrc);
	gl.compileShader(vertexShader);
	var fragmentShader = gl.createShader(gl.FRAGMENT_SHADER);
	var result = this.fragmentBloomShaderSrc;
	gl.shaderSource(fragmentShader, result);
	gl.compileShader(fragmentShader);
	this.shaderProgramBloom = gl.createProgram();
...
BloomRenderer.prototype.initializeShadersBloomX = function (gl) {
	var vertexShader = gl.createShader(gl.VERTEX_SHADER);
	gl.shaderSource(vertexShader, RendererVertexShaderSrc);
	gl.compileShader(vertexShader);
	var fragmentShader = gl.createShader(gl.FRAGMENT_SHADER);
...
BloomRenderer.prototype.initializeShadersBloomY = function (gl) {
	var vertexShader = gl.createShader(gl.VERTEX_SHADER);
	gl.shaderSource(vertexShader, RendererVertexShaderSrc);
	gl.compileShader(vertexShader);
	var fragmentShader = gl.createShader(gl.FRAGMENT_SHADER);
...
BloomRenderer.prototype.initializeFrameBuffers = function (gl) {
...
BloomRenderer.prototype.renderOnBuffers = function (scene, frameBuffer, frameBufferPost, texture, gl) {
